...
namespace BetterInfestations
{
    ...
    public class IncidentWorker_WorldInfestation : IncidentWorker
    {
        protected override bool CanFireNowSub(IncidentParms parms)
        {
            ...
            return Patches.Patch_FactionGenerator_GenerateFactionsIntoWorldLayer.RandomInfestationTile(out _);
        }
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            int index;
            if (Patches.Patch_FactionGenerator_GenerateFactionsIntoWorldLayer.RandomInfestationTile(out index))
            {
        ...
        protected override bool TryExecuteWorker(IncidentParms parms)
        {
            if (BetterInfestationsMod.settings != null)
            {
                if (BetterInfestationsMod.settings.raidsAllowed)
                {
                    ...
                    if (list.Count > 0)
                    {
                        SendStandardLetter("BI_LetterLabelInsectRaid".Translate(),
                            "BI_LetterTextInsectRaid".Translate(Faction.OfInsects.Name.ApplyTag(Faction.OfInsects)),
                            LetterDefOf.ThreatBig, parms, list[0]);
                    }
                    else
                    {
                        Log.Warning("[BetterInfestations] Generated an insect raid with 0 pawns.");
                        return false;
                    }
                    Find.TickManager.slower.SignalForceNormalSpeedShort();
        ...
        public static List<Pawn> GenerateInsects(int tile, float points)
        {
            ...
            for (int i = 0; i < 500; i++)
            {
                ...
                Pawn item = PawnGenerator.GeneratePawn(
                    new PawnGenerationRequest(pawnKindDef, Faction.OfInsects, PawnGenerationContext.NonPlayer, tile));
                list.Add(item);
                accumPoints += pawnKindDef.combatPower;
            }
            return list;
