...
namespace BetterInfestations
{
        ...
        private IntVec3 FindGroundSleepSpotFor(Pawn pawn)
        {
            ...
            if (hive != null)
            {
                if (pawn.CanReach(hive.Position, PathEndMode.OnCell, Danger.Deadly, true, true, TraverseMode.PassDoors))
                {
                    ...
                    {
                        int radius = (i == 0) ? 4 : 8;
                        if (CellFinder.TryRandomClosewalkCellNear(hive.Position, map, radius, out IntVec3 result,
                            (IntVec3 x) => !x.IsForbidden(pawn)
                                        && !x.GetTerrain(map).avoidWander
                                        && pawn.Map.reservationManager.CanReserve(pawn, x)))
                        {
                            return result;
                        }
                    }
            ...
            // Backup: Try to find any valid sleep spot, ensuring it isn't reserved
            for (int attempts = 0; attempts < 5; attempts++)  // Limit attempts to avoid infinite loops
            {
                IntVec3 fallbackCell = CellFinder.RandomClosewalkCellNearNotForbidden(pawn, 4);
                if (pawn.Map.reservationManager.CanReserve(pawn, fallbackCell))
                {
                    return fallbackCell;
                }
            }

            // If no valid cell is found, return an invalid cell to prevent job conflicts
    ...
    public class JobGiver_Gather : ThinkNode_JobGiver
    {
        public static Job ForceJob(Pawn pawn)
        {
            ...
            //added for stockpile issue
            if (pos.IsValid && pos.GetZone(pawn.Map) is Zone_Stockpile)
                return null;
    ...
    public class JobGiver_Sapper : ThinkNode_JobGiver
    {
        public static Job ForceJob(Pawn pawn, IntVec3 c)
        {
            ...
            using (PawnPath pawnPath = pawn.Map.pathFinder.FindPathNow(start: pawn.Position, target: c, traverseParms: TraverseParms.For(pawn, Danger.Deadly, TraverseMode.PassAllDestroyableThings, false), peMode: PathEndMode.OnCell))
            {
